...
\subsubsection{Pemetaan Komponen Logis PAP, PIP, PDP, dan PEP}
\label{subsubsec:pemetaan-pap-pip-pdp-pep}

Proses administrasi, pengambilan keputusan, penyediaan informasi, dan penegakan \textit{policy} pada rancangan DecMed dapat dipetakan ke peran logis \textit{Policy Administration Point} (PAP), \textit{Policy Information Point} (PIP), \textit{Policy Decision Point} (PDP), dan \textit{Policy Enforcement Point} (PEP). Pemetaan tersebut bersifat logis dan tidak menyatakan pembagian DecMed menjadi empat layanan fisik yang terpisah. Satu komponen implementasi dapat menjalankan lebih dari satu fungsi peran tersebut, sebagaimana dirangkum pada Tabel~\ref{tab:pemetaan-pap-pip-pdp-pep-decmed}.

\begin{table}[H]
	\centering
	\caption{Pemetaan PAP, PIP, PDP, dan PEP pada Rancangan DecMed}
	\label{tab:pemetaan-pap-pip-pdp-pep-decmed}
	\small
	\begin{tabularx}{\textwidth}{%
		...
		|>{\raggedright\arraybackslash}X|}
		\hline
		\textbf{Komponen Logis}
		 & \textbf{Realisasi pada DecMed}
		 & \textbf{Keterangan}                                                                                                                                                                                         \\ \hline

		\textit{Policy Administration Point} (PAP)
		 & \textit{Patient Client}; pengajuan cakupan melalui \textit{Hospital Client}; \textit{Server} PRE pada atenuasi; \textit{caveat} Macaroon
		 & Pasien menetapkan token awal; \textit{Hospital Client} hanya mengajukan cakupan; \textit{Server} PRE menolak perluasan cakupan token induk.                                                                 \\ \hline

		\textit{Policy Information Point} (PIP)
		 & \textit{Caveat} Macaroon; \textit{wallet proof}; \textit{role}/\textit{subrole} dan metadata pada IOTA; status revokasi Redis; konteks permintaan
		 & Atribut kapabilitas dan kelembagaan, metadata objek, status revokasi, serta konteks $q$ (Persamaan~\ref{eq:permintaan-akses}).                                                                              \\ \hline

		\textit{Policy Decision Point} (PDP)
		 & Logika verifikasi \textit{Server} PRE atas $\operatorname{Permit}(q)$ dan $\operatorname{Decision}(q)$ (Persamaan~\ref{eq:permit-akses},~\ref{eq:deny-akses})
		 & Validitas Macaroon dan \textit{EffectiveCapability}; operasi \texttt{Read}/\texttt{Update}; rantai delegasi; \textit{wallet proof}; kecocokan objek; kedaluwarsa; \textit{role}/\textit{subrole}; revokasi. \\ \hline

		\textit{Policy Enforcement Point} (PEP)
		 & \textit{Endpoint} dan \textit{middleware} \textit{Server} PRE; Move \textit{smart contract} pada IOTA
		 & $\operatorname{Deny}$: tolak permintaan, tanpa \textit{re-encryption} dan \texttt{c\_frag}. $\operatorname{Permit}$: lanjutkan operasi \textit{re-encryption}.                                              \\ \hline
	\end{tabularx}
\end{table}

\textit{Server} PRE menjalankan fungsi PDP dan PEP dalam komponen yang sama. Fungsi PIP tersebar pada Macaroon, IOTA, Redis, dan konteks permintaan. Fungsi PAP direalisasikan melalui penerbitan token awal dan pembentukan token turunan, dengan rincian representasi \textit{caveat} pada Subbab~\ref{subsec:rancangan-token-macaroons} dan penegakan otoritas pada Subbab~\ref{subsec:rancangan-penegakan-otoritas}.
