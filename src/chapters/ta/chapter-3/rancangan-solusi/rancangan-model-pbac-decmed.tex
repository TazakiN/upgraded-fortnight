...
% Rancangan model PBAC pada subbab ini direferensikan sebagai \texttt{R-TA-03}. Rancangan ini merealisasikan \texttt{S-TA-01} untuk menjawab \texttt{M-TA-01}.

Berdasarkan hasil analisis pada Subbab \ref{subsec:analisis-pendekatan-model-pbac}, elemen dan relasi pada \textit{Policy Machine} diadaptasi pada rancangan kebijakan akses DecMed. Adaptasi ini digunakan untuk memodelkan hubungan antara pengguna, atribut pengguna, segmen RME, atribut segmen, dan hak akses. Informasi yang direpresentasikan melalui \textit{caveat} Macaroon dan kondisi yang diverifikasi oleh \textit{Server} PRE ditentukan berdasarkan model tersebut.

\subsubsection{Ruang Lingkup Adaptasi Policy Machine}

Elemen kebijakan DecMed didefinisikan sebagai:
...
\begin{description}
    \item[$U$] himpunan pengguna;
    \item[$UA$] himpunan atribut pengguna;
    \item[$O$] himpunan objek yang dilindungi;
    \item[$OA$] himpunan atribut objek; dan
    \item[$PC$] himpunan kelas kebijakan.
\end{description}

Berdasarkan elemen tersebut, model kebijakan DecMed dirumuskan sebagai:
...
Elemen $\mathcal{C}$ merupakan perluasan pada rancangan ini untuk merepresentasikan kondisi dinamis yang ditegakkan melalui Macaroon dan \textit{Server} PRE, seperti masa berlaku, rantai delegasi, revokasi, dan bukti kepemilikan identitas aktor pada rantai delegasi. Elemen tersebut tidak dinyatakan sebagai penambahan terhadap spesifikasi asli \textit{Policy Machine}, tetapi sebagai pemetaan kondisi implementasi yang diperlukan oleh DecMed.

Hanya elemen kebijakan, relasi penugasan, dan asosiasi yang relevan terhadap kontrol akses RME yang diadaptasi. \textit{Prohibition}, \textit{obligation}, fungsi administratif, atau implementasi graf kebijakan \textit{Policy Machine} secara lengkap tidak dicakup pada rancangan ini.

\subsubsection{Pengguna dan Atribut Pengguna}

Himpunan pengguna $U$ berisi pengguna DecMed yang diidentifikasi berdasarkan alamat dompet IOTA:
...
Elemen pada himpunan tersebut merepresentasikan alamat dompet aktual, bukan hanya nama peran. Sebagai contoh, $u_{\text{dokter-A}}$ dan $u_{\text{dokter-B}}$ merupakan dua pengguna yang berbeda meskipun keduanya memiliki atribut \textit{subrole} dokter.

Atribut pengguna dibagi menjadi atribut kelembagaan dan atribut kapabilitas:
...
Atribut objek $OA$ digunakan untuk mengelompokkan segmen berdasarkan beberapa tingkat berikut:
...
Hubungan objek dan atribut objek membentuk hierarki:
...
\begin{description}
    \item[$f(s)$] kategori fungsi dari segmen $s$;
    \item[$d(s)$] kategori dataset dari segmen $s$;
    \item[$r(s)$] identitas episode RME;
    \item[$p(s)$] alamat pasien pemilik RME; dan
    \item[$h(s)$] rumah sakit tempat episode pelayanan berlangsung.
\end{description}

Atribut kategori fungsi dan dataset harus dibentuk dalam konteks pasien atau episode RME tertentu. Sebagai contoh, atribut diagnosis pada RME milik pasien A berbeda dengan atribut diagnosis pada RME milik pasien B. Pemisahan tersebut diperlukan agar asosiasi terhadap fungsi diagnosis pada satu RME tidak memberikan akses terhadap fungsi diagnosis pada seluruh pasien.
...
Himpunan kategori dataset yang digunakan pada rancangan ini adalah:
...
Sementara itu, kategori fungsi mengikuti segmentasi data yang telah dijelaskan pada Subbab \ref{subsec:rancangan-segmentasi-data-rme}, seperti anamnesis, pemeriksaan fisik, pemeriksaan psikologis, diagnosis, pemeriksaan penunjang, terapi, laboratorium, peresepan, dan \textit{dispensing}.
...
$AR_{\operatorname{eff}}(\tau)$ merupakan hak akses efektif token, sedangkan $OA_{\operatorname{scope}}(\tau)$ merupakan himpunan atribut objek yang termasuk dalam cakupan token.

Cakupan objek token didefinisikan sebagai:
...
$D_{\operatorname{eff}}(\tau)$ merupakan irisan seluruh batasan kategori dataset pada token, sedangkan $F_{\operatorname{eff}}(\tau)$ merupakan irisan seluruh batasan kategori fungsi:
...
Penggunaan operasi irisan memastikan bahwa penambahan \textit{caveat} pada token turunan hanya dapat mempertahankan atau mempersempit cakupan akses. Penambahan batasan baru tidak dapat mengembalikan kategori dataset atau fungsi yang telah dikecualikan oleh token induk.
...
\subsubsection{Kondisi Kebijakan Akses}
...
\begin{table}[!ht]
    \centering
    \caption{Kondisi Kebijakan Akses DecMed}
    ...
    \begin{tabular}{|p{0.24\textwidth}|p{0.32\textwidth}|p{0.31\textwidth}|}
        ...
        Otoritas kelembagaan
         ...
         & Membatasi operasi tulis sesuai kewenangan tenaga medis.          \\ \hline
...
\subsubsection{Rancangan Kebijakan Delegasi}
...
Hubungan antara token turunan dan token induk disimpan melalui \texttt{parent\_token\_hash}. Sementara itu, perubahan aktor direpresentasikan dengan menambahkan \texttt{delegated\_by} dan \texttt{delegated\_to}. Karena seluruh \textit{caveat} token induk tetap dipertahankan, proses delegasi hanya dapat menambahkan batasan baru dan tidak dapat menghapus batasan sebelumnya.
...
\begin{description}
    \item[$u$] alamat dompet pengguna yang melakukan permintaan;
    \item[$h$] identitas rumah sakit;
    \item[$p$] alamat pasien;
    \item[$r$] identitas episode RME;
    \item[$d$] kategori dataset;
    \item[$f$] kategori fungsi;
    \item[$a$] operasi yang diminta;
    \item[$t$] waktu permintaan;
    \item[$\tau$] token Macaroon; dan
    \item[$\pi$] bukti kepemilikan \textit{wallet}.
\end{description}

Permintaan diberikan apabila seluruh kondisi kebijakan terpenuhi:
...
\begin{equation}
    \begin{split}
        ...
        a \neq \texttt{Update}
        \lor
        \operatorname{RoleScopeValid}(u,d,f)
        \right)                                                   \\
                & \land
        \left(
        a \neq \texttt{Update}
        \lor
        \operatorname{RmeMatch}(r,\tau)
        \right).
    \end{split}
...
Prinsip \textit{deny by default} diterapkan melalui fungsi tersebut. Kepemilikan token saja tidak cukup untuk memperoleh akses. Token harus valid, permintaan harus berasal dari salah satu aktor pada rantai delegasi, objek harus berada dalam cakupan token, token tidak boleh kedaluwarsa atau direvokasi, dan operasi tulis harus sesuai dengan \textit{subrole} tenaga medis.

\subsubsection{Pemetaan Model Kebijakan ke Komponen DecMed}

Pemetaan elemen model kebijakan terhadap komponen implementasi ditunjukkan pada Tabel \ref{tab:pemetaan-policy-machine-decmed}.

\begin{table}[!ht]
    \centering
    \caption{Pemetaan Model Kebijakan terhadap Komponen DecMed}
...
Berdasarkan rancangan tersebut, model CapBAC yang telah digunakan oleh DecMed tidak digantikan oleh model PBAC. Macaroon tetap berfungsi sebagai token kapabilitas yang dibawa oleh pengguna, sedangkan elemen dan kondisi yang harus diperiksa untuk menafsirkan kapabilitas tersebut ditentukan oleh model PBAC. Dengan demikian, cara otoritas dibawa oleh token dijelaskan melalui CapBAC, sedangkan kebijakan yang menentukan validitas penggunaan otoritas tersebut dijelaskan melalui PBAC.

Rincian representasi kebijakan dalam bentuk \textit{caveat} dijelaskan pada Subbab \ref{subsec:rancangan-token-macaroons}. Mekanisme evaluasi dan penegakannya pada \textit{Server} PRE dijelaskan pada Subbab \ref{subsec:rancangan-penegakan-otoritas}.
