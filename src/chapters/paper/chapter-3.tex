...
\subsection{Offline Delegation and Request Verification}
...
Evaluation traced each stated requirement to implementation evidence and a test scenario. Twelve functional tests (\texttt{P-KF-01}--\texttt{P-KF-12}) were executed manually over outpatient and inpatient workflows. The inpatient path additionally exercised the discharge-planning segment. Thirteen nonfunctional tests (\texttt{P-KNF-01}--\texttt{P-KNF-13}) combined Rust unit/integration tests run with \texttt{cargo test} and client-level scenarios. These tests concern authorization correctness, traceability, and selected negative cases; they are not penetration tests or performance benchmarks.

Table~\ref{tab:evaluation-summary} groups the 25 documented scenarios without treating each user-interface step as a separate scientific result. The thesis evaluation records all mapped functional and nonfunctional requirements as fulfilled.
...
This work extended DecMed with category-based EMR segmentation and Macaroon capabilities whose caveats constrain operation, patient, episode, hospital, data scope, expiry, and delegation chain. PRE-mediated attenuation prevents a requested child from exceeding its parent, while wallet proof, role/subrole validation, Redis revocation, and IOTA audit records bind the capability to the implemented workflow. Across 12 functional and 13 authorization-oriented tests, the documented prototype satisfied its mapped requirements and rejected the tested out-of-scope, non-monotonic, expired, malformed-chain, wallet-invalid, and revoked cases.

The evidence supports functional correctness for the evaluated prototype, not a general claim of security or efficiency. Future work should validate the design in hospital-integrated and inter-facility workflows, map segments to SATUSEHAT/FHIR representations, and evaluate performance, availability, scalability, and security more extensively.
